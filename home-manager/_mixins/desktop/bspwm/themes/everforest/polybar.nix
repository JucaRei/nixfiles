    ...
    polybar = {
      ...
      settings = {
        ...
        "bar/bar" = {
          ...
          modules-center = "";
          modules-right =
            "sep network blok2 audio blok memory_bar blok bi cpu_bar bd blok date blok powermenu";
        ...
        "module/pulseaudio-control-output" = {
          ...
          exec = ''
            ${pkgs.pulseaudio-control} --format '$VOL_ICON\ \  $VOL_LEVEL% \($NODE_NICKNAME\)' --color-muted "${
              builtins.replaceStrings [ "#" ] [ "" ] theme.disabled
            }" --icons-volume " , " --icon-muted "ﱝ " --node-nicknames-from "device.profile.name" --node-nickname "alsa_output.pci-0000_00_1f.3.analog-stereo:built-in" listen'';
        ...
        "module/audio" = {
          ...
          click-right =
            "bspc rule -a Pavucontrol -o state=floating follow=on center=true && pavucontrol";
        };
        "module/bspwm" = {
          type = "internal/bspwm";
        ...
        "module/mpd" = {
          ...
          interval = 2;
          format-online = ''
            <icon-repeat> %{F#9ece6a}[%{F-} %{A1:bspc rule -a org.wezfurlong.wezterm -o state=floating follow=on center=true && wezterm start -- " ncmpcpp ":}<label-song>%{A} %{F#9ece6a}]%{F-}'';
          format-offline = "";
          label-song = "%title%";
