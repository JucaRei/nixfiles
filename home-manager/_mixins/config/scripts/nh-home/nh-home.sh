...
if [ -e "${HOME}/.dotfiles/nixfiles" ]; then
    ...
    case $1 in
        build) nh home build "${HOME}/.dotfiles/nixfiles/" -- --cores "${build_cores}";;
        switch) nh home switch --backup-extension backup "${HOME}/.dotfiles/nixfiles/" -- --cores "${build_cores}";;
    esac
else
    echo "ERROR! No nixfiles found in ${HOME}/.dotfiles/nixfiles"
fi
