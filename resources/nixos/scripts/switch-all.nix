...
pkgs.writeScriptBin "switch-all" ''
  ...
  if [ -e $HOME/.dotfiles/nixfiles ]; then
    ...
    ${pkgs.unstable.nh}/bin/nh os switch $HOME/.dotfiles/nixfiles -- --show-trace -vL --cores $build_cores
    echo "Switching Home Manager with $build_cores cores"
    ${pkgs.unstable.nh}/bin/nh home switch --backup-extension backup $HOME/.dotfiles/nixfiles/ -- --show-trace -vL --impure --cores $build_cores
  else
