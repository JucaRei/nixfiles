...
if [ -e "${HOME}/.dotfiles/nixfiles" ]; then
    ...
    mkdir -p "${HOME}/virtualmachines/nixos-${1}" 2>/dev/null
    cp "result/iso/${ISO}" "${HOME}/virtualmachines/nixos-${1}/nixos.iso"
    chown "${USER}": "${HOME}/virtualmachines/nixos-${1}/nixos.iso"
    chmod 644 "${HOME}/virtualmachines/nixos-${1}/nixos.iso"
