...
let
  inherit (lib) mkIf types optionalString optional mkMerge;
  inherit (lib.${namespace}) mkBoolOpt mkOpt;
  cfg = config.${namespace}.hardware.cpu;
in
{
  options.${namespace}.hardware.cpu = {
    enable = mkBoolOpt false "Enable or disable CPU specific options";
    hardenKernel = mkBoolOpt false "Enable kernel hardening options";
    improveTCP = mkOpt types.bool false "Enable TCP hardening options";
    enableKvm = mkOpt types.bool false "Enable KVM options";
    cpuVendor = mkOpt (types.srt (types.enum [ "intel" "amd" "other" ])) "intel" "Vendor for the CPU, meant mostly for x86/x86_64 systems.";
  };
