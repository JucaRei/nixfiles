...
let
  ...
  cfg = config.${namespace}.services.writefreely;
...
in
{
  options.${namespace}.services.writefreely = {
    ...
    database = {
      ...
      createLocally = mkOption {
        ...
        default = false;
        description = lib.mdDoc ''
          When `excalibur.services.writefreely.database.type` is set to
  ...
  config = mkIf cfg.enable {
    assertions = [
      {
        assertion = cfg.host != "";
        message = "excalibur.services.writefreely.host must be set";
      }
      {
        assertion = isMysqlLocal -> cfg.database.passwordFile != null;
        message = "excalibur.services.writefreely.database.passwordFile must be set if excalibur.services.writefreely.database.createLocally is set to true";
      }
      {
        assertion = isSqlite -> !cfg.database.createLocally;
        message = "excalibur.services.writefreely.database.createLocally has no use when excalibur.services.writefreely.database.type is set to sqlite3";
      }
    ];
