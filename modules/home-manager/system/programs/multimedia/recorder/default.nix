...
let
  ...
  cfg = config.system.programs.multimedia.audio-recorder;
in
{
  options.system.programs.multimedia.audio-recorder = {
    enable = mkOption {
      default = false;
      type = types.bool;
    };
  ...
  config = mkIf cfg.enable {
    ...
    dconf.settings = {
      "apps/audio-recorder" = {
        ...
        folder-name = "${config.xdg.userDirs.music}/recordings";
        keep-on-top = true;
        level-bar-value = 2;
        media-format = "Lossless WAV 22KHz";
    ...
    systemd.user.tmpfiles.rules = [
      "d ${config.xdg.userDirs.music}/recordings 0755 ${username} users - -"
    ];
  };
}
