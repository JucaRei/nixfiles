...
let
  ...
  cfg = config.programs.graphical.apps.terminal.urxvt;
  ...
  termDesktop = pkgs.writeTextFile {
    ...
    text = ''
      [Desktop Entry]
      Exec=/home/${username}/.local/bin/term
...
{
  options = {
    programs.graphical.apps.terminal.urxvt = {
      enable = mkOption {
        default = false;
        type = types.bool;
        description = "Enables rxvt terminal and daemon.";
      };
    };
  };
