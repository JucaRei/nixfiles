{ pkgs, lib ? pkgs.lib, config, nixGLWrapper, ... }:
let
  inherit (pkgs.stdenv) isLinux;
  inherit (lib) mkIf mkEnableOption;
  cfg = config.apps.graphical.terminal.alacritty;
in
{
  options = {
    apps.graphical.terminal.alacritty = {
      enable = mkEnableOption "Enable's alacritty with configs.";
    };
  };
