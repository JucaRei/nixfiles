{ lib, pkgs, config, nixGLWrapper, ... }:
let
  inherit (pkgs.stdenv) isLinux;
  inherit (lib) mkOption mkIf types;
  cfg = config.apps.graphical.terminal.blackbox;
in
{
  options.apps.graphical.terminal.blackbox = {
    enable = mkOption {
      default = false;
      type = types.bool;
  ...
  config = mkIf cfg.enable {
    ...
    home = {
      ...
      packages = with pkgs; [ (nixGLWrapper blackbox-terminal) ];
    };
  };
}
