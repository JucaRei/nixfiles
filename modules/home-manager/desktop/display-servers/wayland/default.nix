  ...
  config = mkIf (backend == "wayland") {
    ...
    home = {
      sessionVariables = {
        ...
        __GLX_VENDOR_LIBRARY_NAME = mkIf isNixOS (
          if hasNvidia || hasGpuFallback == "nvidia" then "nvidia" else null
        );
        __NV_PRIME_RENDER_OFFLOAD = mkIf isNixOS (
        ...
        IGPU_CARD = mkIf (
          !isArm
        ) "$(${pkgs.uutils-coreutils-noprefix}/bin/readlink -f /dev/dri/by-path/pci-0000:00:02.0-card)";
        DGPU_CARD = mkIf (
          !isArm
        ) "$(${pkgs.uutils-coreutils-noprefix}/bin/readlink -f /dev/dri/by-path/pci-0000:01:00.0-card)";
      };

      # Runtime detection for non-NixOS (Wayland-focused, with ARM fallback)
      activation = {
        setWaylandVars = mkIf (!isNixOS) (
