...
- **MPV & Suporte a Vídeos via SMB / Samba (`gvfs-smb.lua` e `gvfs-fuse.service`)**:
  ...
  - **Daemon FUSE do GVfs (`systemd.user.services.gvfs-fuse`)**: Adicionado serviço systemd de usuário em `modules/home-manager/system/programs/file-manager/thunar/default.nix` que supervisiona `${pkgs.gvfs}/libexec/gvfsd-fuse -f %t/gvfs`, expondo compartilhamentos montados pelo GVfs em `/run/user/<uid>/gvfs/smb-share:server=...,share=.../` de forma persistente.
...
- **Aceleração de Vídeo / VA-API no Host `nitro` (Debian Standalone - Intel UHD 630 / NVIDIA GTX 1050)**:
  ...
  - **Configuração Declarativa no Nitro**: Exportados declarativamente `LIBVA_DRIVER_NAME = "iHD"` e `LIBVA_DRIVERS_PATH = "${pkgs.intel-media-driver}/lib/dri:/usr/lib/x86_64-linux-gnu/dri:/usr/lib/dri"` em `home-manager/hosts/nitro/default.nix`, e adicionados `pkgs.intel-media-driver` e `pkgs.libva-utils` (`vainfo`) em `home.packages`. O `vainfo` valida com sucesso todos os perfis de decodificação/codificação por hardware (H.264, HEVC 8/10-bit, VP9, JPEG).
