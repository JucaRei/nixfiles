...
- **Suporte a Binários Nativos da Distro vs Compilação Nix (`useSystemPackage` / `installPackage`)**:
  ...
  - Implementadas as opções `installPackage = bool` (padrão `true`) e atalho `useSystemPackage = bool` (padrão `false`):
    - **MPV (`multimedia/mpv`)**:
      ...
      - **Integração GLX em Distros Híbridas Standalone (`!isNixOS`)**: Configurado wrapper executável `mpv` com `env __GLX_VENDOR_LIBRARY_NAME=mesa /usr/bin/mpv` em `home.packages` e `.desktop` para evitar conflito com GLX proprietário da NVIDIA na GPU integrada tanto no terminal quanto em subprocessos de miniaturas do `thumbfast`. O comando `mpv-nvidia` continua disponível para forçar a GPU dedicada via PRIME Offload.
