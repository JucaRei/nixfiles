...
- **Estética Polybar, BSPWM e Picom (Clean & Native Rice)**:
  - **Picom**: Otimizado para máxima eficiência (`use-damage = true`, `glx-no-stencil = true`, `glx-no-rebind-pixmap = true`) sem animações pesadas que consumiam CPU. Sombras suaves Catppuccin Crust (`#11111b`, opacidade 0.55, raio 14), cantos arredondados modernos (10px a 12px), fading ágil (~150ms) e transparências sutis (0.98 ativa, 0.90 inativa, 0.92 Alacritty).
  - **Polybar**: Barra flutuante moderna (`width = 100%:-28`, `offset-x = 14`, `offset-y = 8`, `height = 34`, `radius = 12`) com fundo Catppuccin Base semi-translúcido (`#e61e1e2e`) e borda sutil. Construída exclusivamente com módulos nativos (`internal/bspwm`, `internal/xwindow`, `internal/date`, `internal/cpu`, `internal/memory`, `internal/pulseaudio`, `internal/battery`) e sem dependência do estilo ou scripts em loops do gh0stzk, mantendo o consumo de CPU em idle próximo a 0%.
  - **BSPWM**: Padding superior calibrado para a barra flutuante (`top_padding = 48`, margens 10), bordas Catppuccin Mauve (`#cba6f7`), Surface1 (`#45475a`) e Surface0 (`#313244`).
- **Rede Dinâmica (Polybar) & Auto-switching (Rocinante)**:
  - **Polybar**: Módulo de rede leve (`scripts.networkScript`) identifica automaticamente Wi-Fi ou Cabo instantaneamente via `nmcli` sob demanda (intervalo 4s) sem loops contínuos.
