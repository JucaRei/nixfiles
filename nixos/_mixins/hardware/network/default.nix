...
let
  ...
  inherit (lib.types) nullOr enum bool str;
...
in
{
  options.hardware.network = {
    enable = mkOption {
      type = bool;
      default = false;
      description = mdDoc "Wheater enable network for device";
    };
    networkOpt = mkOption {
      type = nullOr (enum [ "network-manager" "wpa-supplicant" ]);
      default = null; # "network-manager";
      description = mdDoc "Default network option.";
    };
    exclusive-locallan = mkOption {
      type = bool;
      default = false;
      description = mdDoc "Wheter enable exclusive-lan.";
    };
    powersave = mkOption {
      default = false;
      type = bool;
      description = mdDoc "Enable powersave for wifi.";
    };
    wakeonlan = mkOption {
      type = bool;
      default = false;
