...
disks=(/dev/sda) # Replace with actual device names
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    echo "Creating new partition table on /dev/sda..."
    parted /dev/sda -- mklabel gpt
    parted /dev/sda -- mkpart ESP fat32 1MiB 512MiB
    parted /dev/sda -- set 1 esp on
    parted /dev/sda -- set 1 boot on
    parted /dev/sda -- mkpart primary linux-swap 512MiB 1536MiB
    parted /dev/sda -- mkpart primary 1536MiB 100%

    mkfs.fat -F 32 -n ESP /dev/sda1
    mkswap -f /dev/sda2 --label swap
    # --encrypted --compression=lz4:0 --discard
    keyctl link @u @s
    bcachefs format -f --fs_label=root --label=root --encrypted /dev/sda3
