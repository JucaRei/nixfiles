...
in
{
  config = {
    ...
    system = lib.mkIf isISO { stateVersion = lib.mkForce lib.trivial.release; };
    ...
    environment = {
      ...
      systemPackages = lib.optionals isWorkstationISO [ pkgs.gparted pkgs.vscode-fhs ];
    };

    # All workstation configurations for live media are below.
    isoImage = lib.mkIf isWorkstationISO { edition = lib.mkForce "${desktop}"; };
